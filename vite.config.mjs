...
async function deepseekReply({ messages, memory, env }) {
  ...
  if (!response.ok) {
    ...
    const error = new Error(`DeepSeek ${response.status}: ${String(errMsg).slice(0, 200)}`);
        ...
        configureServer(server) {
          server.middlewares.use("/api/jimmy", async (req, res) => {
            ...
            try {
              ...
              const reply = await deepseekReply({ messages: body.messages, memory, env });
              sendJson(res, 200, { reply });
            } catch (error) {
              sendJson(res, error.status || 500, { error: error.message || "Jimmy AI failed" });
            }
          });
        },
      },
